...
function shouldShowShelfHoverCue(value) {
  ...
  if (!shelfHoverCue.guide) {
    ...
    if (!window.shelfManager || !window.shelfManager.canInteract || !window.shelfManager.canInteract()) return false;
    if (window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) return false;
    if (!window.shelfManager.getMode || window.shelfManager.getMode() !== 'side') return false;
  }
...
function idleGuidePointerDown(e) {
  ...
  window.idleGuideInteraction.lastX = e.clientX;
  window.idleGuideInteraction.lastY = e.clientY;
  window.idleGuideInteraction.lastT = performance.now();
  window.idleGuideInteraction.pointerX = e.clientX / Math.max(1, window.idleGuideW || innerWidth);
...
function idleGuidePointerMove(e) {
  ...
  var dt = Math.max(1 / 120, Math.min(0.08, (now - window.idleGuideInteraction.lastT) / 1000 || 1 / 60));
  var dx = e.clientX - window.idleGuideInteraction.lastX;
  var dy = e.clientY - window.idleGuideInteraction.lastY;
  var rx = -dy * 0.0032;
  var ry = dx * 0.0034;
  window.idleGuideInteraction.rotX += rx;
  window.idleGuideInteraction.rotY += ry;
  window.idleGuideInteraction.angle += ry * 0.22;
  window.idleGuideInteraction.spinX = clampIdleGuideSpin(rx / dt * 0.46);
  window.idleGuideInteraction.spinY = clampIdleGuideSpin(ry / dt * 0.46);
  window.idleGuideInteraction.velocity = Math.sqrt(window.idleGuideInteraction.spinX * window.idleGuideInteraction.spinX + window.idleGuideInteraction.spinY * window.idleGuideInteraction.spinY);
  window.idleGuideInteraction.lastX = e.clientX;
  window.idleGuideInteraction.lastY = e.clientY;
...
function idleGuideWheel(e) {
  ...
  guide.pointerX = e.clientX / Math.max(1, window.idleGuideW || innerWidth);
  guide.pointerY = e.clientY / Math.max(1, window.idleGuideH || innerHeight);
...
function resizeIdleGuideCanvas() {
  ...
  window.idleGuideCanvas.width = Math.max(1, Math.floor(window.idleGuideW * idleGuideDpr));
  window.idleGuideCanvas.height = Math.max(1, Math.floor(window.idleGuideH * idleGuideDpr));
  window.idleGuideCanvas.style.width = window.idleGuideW + 'px';
  window.idleGuideCanvas.style.height = window.idleGuideH + 'px';
  idleGuideCtx.setTransform(idleGuideDpr, 0, 0, idleGuideDpr, 0, 0);
  idleGuideParticles = [];
  resetIdleGuideTrails();
  if (!IDLE_GUIDE_BACKGROUND_ENABLED) return;
  var minDim = Math.min(window.idleGuideW, window.idleGuideH);
  var maxDim = Math.max(window.idleGuideW, window.idleGuideH);
  var count = window.idleGuideW < 800 ? 150 : 240;
...
function drawIdleGuideFrame() {
  ...
  if (!show) {
    idleGuideCtx.clearRect(0, 0, window.idleGuideW, window.idleGuideH);
    resetIdleGuideTrails();
  ...
  if (!idleShow) {
    ctx.clearRect(0, 0, window.idleGuideW, window.idleGuideH);
    resetIdleGuideTrails();
  ...
  var cx = window.idleGuideW * 0.5;
  var cy = window.idleGuideH * 0.50;
  var guide = window.idleGuideInteraction;
  if (!guide.dragging) {
  ...
  var halo = ctx.createRadialGradient(cx, cy, 0, cx, cy, Math.min(window.idleGuideW, window.idleGuideH) * ((0.36 + breathe * 0.035 + guide.press * 0.018) * zoom));
  ...
  ctx.fillStyle = halo;
  ctx.fillRect(0, 0, window.idleGuideW, window.idleGuideH);

  var ringPts = [];
  var pointerX = guide.pointerX * window.idleGuideW;
  var pointerY = guide.pointerY * window.idleGuideH;
  ...
  var depth = Math.max(520, Math.min(window.idleGuideW, window.idleGuideH) * 0.92);
  for (var i = 0; i < idleGuideParticles.length; i++) {
    ...
    if (p.ring) {
      ...
      var driftX = ((p.cx - 0.5) * window.idleGuideW * 0.92 + Math.cos(localA) * (12 + p.wobbleAmp * 0.28) + wobble * 0.28) * zoom;
      var driftY = ((p.cy - 0.5) * window.idleGuideH * 0.72 + Math.sin(localA * 0.8 + p.phase * 0.2) * (12 + p.wobbleAmp * 0.24)) * zoom;
  ...
  for (var j = 0; j < ringPts.length; j += 3) {
    ...
    if (dist > Math.min(window.idleGuideW, window.idleGuideH) * 0.17) continue;
  ...
  if (guide.focus > 0.03 || spinEnergy > 0.05) {
    var orbitR = Math.min(window.idleGuideW, window.idleGuideH) * (0.305 + guide.press * 0.018) * zoom;
  ...
  if (guide.focus > 0.03) {
    ...
    var handleR = Math.min(window.idleGuideW, window.idleGuideH) * (0.315 + breathe * 0.012 + guide.press * 0.012) * zoom;
...
var visualGuideStepsDiy = [
  ...
  {
    selector: '#window.playlist-panel',
    kicker: '03 / Library',
    title: '左侧是完整歌单和队列',
    body: '靠近左侧边缘可以打开歌单/队列面板，在这里管理队列、个人歌单和播客。'
  },
  {
    selector: '#window.fx-panel',
...
function maybeRunStartupVisualGuide(source) {
  ...
  setTimeout(function(){
    if (!visualGuideWasSeen() || source === 'manual') startVisualGuide({ source: window.source || 'startup' });
...
function prepareVisualGuideStep(step) {
  ...
  if (step && step.selector === '#window.fx-panel') setPeek(fxPanel, true, 'window.fx');
  else if (fxPanel && !visualGuideState.fxWasPeek) setPeek(fxPanel, false, 'window.fx');
...
function guideTargetRect(step) {
  ...
  var isFullscreenDiyStep = !!(step && step.selector === '#diy-mode-btn' && (window.desktopRuntimeState.fullscreen || desktopFullscreenActive || document.fullscreenElement || document.body.classList.contains('desktop-fullscreen')));
...
async function startGestureControl() {
  ...
  try {
    ...
    window.showToast('手势已开启: 手掌推开 · 捏合旋转 · 握拳收束');
    showGestureHUD('待命', 0, '把手放进视野');
  } catch (e) {
    console.warn('Gesture failed:', e);
    window.showToast('手势启动失败 (需要摄像头权限)');
    window.fx.cam = 'off';
...
function onHandLost() {
  ...
  gestureGrip.target = 0;
  window.uniforms.uHandActive.value *= 0.9;
  if (window.uniforms.uHandActive.value < 0.02) window.uniforms.uHandActive.value = 0;
...
function processHandFrame(rawLm) {
  ...
  var gripTarget = window.clampRange(1 - openness, 0, 1);
  ...
  var cur = window.uniforms.uHandXY.value;
  cur.x += (handLocalX - cur.x) * 0.48;
  cur.y += (handLocalY - cur.y) * 0.48;
  var tgtActive = 0.44 + openness * 0.56;
  window.uniforms.uHandActive.value += (tgtActive - window.uniforms.uHandActive.value) * 0.26;
  ...
  if (isPinch && !pinchState.active) {
    ...
    if (gestureGrip.lastState !== 'fist') {
      gestureGrip.pulse = 1;
      window.uniforms.uBurstAmt.value = Math.max(window.uniforms.uBurstAmt.value, 0.26);
    }
    ...
    if (gestureGrip.lastState === 'fist' && openness > 0.58) {
      window.uniforms.uBurstAmt.value = Math.max(window.uniforms.uBurstAmt.value, 0.18);
    }
...
function drawHandSkeleton(lm, isPinch, openness, isFist) {
  ...
  openness = window.clampRange(openness == null ? 1 : openness, 0, 1);
  var palm = palmCenter(lm);
  ...
  for (var r = 0; r < 3; r++) {
    ...
    ctx.beginPath();
    ctx.arc(px, py, ringR + r * 13 + Math.sin(window.uniforms.uTime.value * 1.5 + r) * 2, 0, Math.PI * 2);
    ctx.stroke();
  }
  ...
  for (var i = 0; i < tips.length; i++) {
    ...
    var dist = Math.sqrt(dx * dx + dy * dy);
    var beamAlpha = window.clampRange(0.26 - dist / 720, 0.045, 0.18) * (0.55 + openness * 0.45);
...
function tickGestureRotation(dt) {
  ...
  if (window.uniforms.uGestureGrip) window.uniforms.uGestureGrip.value = window.clampRange(gestureGrip.value + gestureGrip.pulse * 0.16, 0, 1);
  // hand active 自然衰减 (无手时)
  if (gestureActive && handLmSmooth && performance.now() - handLmLastSeen > 200) {
    window.uniforms.uHandActive.value *= 0.94;
...
function refreshMainRendererViewport(reason) {
  ...
  if (typeof requestStageLyricCameraSnap === 'function' && (window.desktopRuntimeState.fullscreen || document.fullscreenElement)) {
...
window.addEventListener('resize', function(){
  ...
  if (window.desktopRuntimeState.fullscreen || desktopFullscreenActive || document.fullscreenElement || document.body.classList.contains('desktop-fullscreen')) layoutFullscreenDiyZone();
});
document.addEventListener('keydown', function(e){
  ...
  if (e.code === 'Space') {
    if (window.freeCamera && window.freeCamera.active) { e.preventDefault(); return; }
  ...
  else if (e.code === 'ArrowRight') window.nextTrack();
  else if (e.code === 'ArrowLeft')  window.prevTrack();
    ...
    if (window.miniQueueOpen) { closeMiniQueue(); return; }
    if (window.shelfManager && window.shelfManager.hasOpenContent()) { window.safeShelfCloseContent('escape-key'); return; }
    window.closeLoginModal(); window.closeUserModal(); toggleFxPanel(false); window.togglePlaylistPanel(false);
  }
  else if (e.code === 'KeyL') { if (!window.immersiveMode) window.toggleLyricsPanel(); }
  else if (e.code === 'KeyP') {
    if (!window.immersiveMode && window.diyPlayerMode) toggleFxPanel();
    else if (!window.immersiveMode) window.showToast('开启 DIY 玩家模式后可打开视觉控制台');
  }
  else if (e.code === 'KeyI') window.toggleImmersiveMode();
  else if (e.code === 'KeyF') window.toggleFullscreen();
});
...
function setPeek(el, on, key) {
  ...
  if (!on && key === 'search' && emptyHomeActive && !window.immersiveMode) return;
  if (!on && key === 'pl' && window.playlistPanelPinned) return;
  if (on && key === 'window.fx') document.body.classList.remove('fullscreen-diy-peek');
  if (on) {
    var wasPeek = el.classList.contains('peek');
    if (peekTimers[key]) { clearTimeout(peekTimers[key]); peekTimers[key] = null; }
    if (key === 'window.fx') el.classList.remove('closing');
    if (key === 'pl' && !wasPeek && !window.playQueue.length && queueViewTab === 'queue') window.switchPlaylistTab('playlists');
    if (key === 'pl' && !wasPeek && window.playQueue.length && window.currentIdx >= 0) {
      if (el.dataset && el.dataset.preserveTabOnOpen === '1') delete el.dataset.preserveTabOnOpen;
      else if (window.queueViewTab !== 'queue') window.switchPlaylistTab('queue');
    ...
    if (key === 'pl' && !wasPeek) {
      scheduleUiWarmTask(function(){
        flushDeferredQueuePanel('window.playlist-panel-peek');
...
window.addEventListener('mousemove', function(e){
  ...
  if (window.immersiveMode) {
    ...
    var shelfCanFocusImm = !!(window.shelfManager && window.shelfManager.canInteract && window.shelfManager.canInteract());
    ...
    else if (window.shelfManager && window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) newFocusImm = 'shelf-detail';
    else if (shelfHoverFocusImm) newFocusImm = 'shelf-side';
    else if (shelfCanFocusImm && window.shelfManager.getMode() === 'stage' && ey > H * 0.55) newFocusImm = 'shelf-stage';
  ...
  if (!window.diyPlayerMode) inFxPanel = inFxFab = inFxBridge = false;
  if (inFxFab || inFxPanel || inFxBridge) setPeek(fp, true, 'window.fx');
  else if (fpOn) setPeek(fp, false, 'window.fx');
  ...
  var shelfCanFocus = !!(window.shelfManager && window.shelfManager.canInteract && window.shelfManager.canInteract());
  if (!shelfCanFocus && !(window.shelfManager && window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent())) {
  ...
  if (queueFocusActive) {
    newFocus = 'queue';
  } else if (window.shelfManager && window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) {
    newFocus = 'shelf-detail';
  } else if (shelfHoverFocus) {
    newFocus = 'shelf-side';
  } else if (shelfCanFocus && window.shelfManager.getMode() === 'stage' && ey > H * 0.55) {
