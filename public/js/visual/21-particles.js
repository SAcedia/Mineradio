...
window.dotTexture = window.makeDotTexture();
...
window.GRID_X = window.coverParticleGridForResolution((typeof window.fx !== 'undefined' && window.fx) ? window.fx.coverResolution : 1.55);
...
window.scheduleCoverResolutionReload = function() {
  ...
  coverResolutionReloadTimer = setTimeout(function(){
    ...
    if (currentCoverSource.kind === 'url') {
      loadCoverFromUrl(currentCoverSource.src, { trackToken: window.trackSwitchToken, fromResolutionChange: true });
    } else if (currentCoverSource.kind === 'data') {
      applyCoverDataUrl(currentCoverSource.src, { trackToken: window.trackSwitchToken, fromResolutionChange: true });
    }
  }, 260);
}
...
var uniforms = {
  ...
  uPixel:      { value: window.renderer.getPixelRatio() },
...
window.installRenderPowerHooks();
...
var material = new THREE.ShaderMaterial({
  uniforms: window.uniforms, vertexShader: vs, fragmentShader: fs,
...
var bloomMaterial = new THREE.ShaderMaterial({
  uniforms: window.uniforms, vertexShader: bloomVs, fragmentShader: bloomFs,
...
window.scene.add(bloomParticles);
...
window.createFloatLayer = function() {
  ...
  var fmat = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uDotTex: window.uniforms.uDotTex,
      uFloatAlpha: window.uniforms.uFloatAlpha,
    },
...
window.effectiveSkullVisualTint = function() {
  ...
  var color = custom
    ? window.fx.visualTintColor
    : (pal.secondary || pal.primary || window.fx.visualTintColor || window.fxDefaults.visualTintColor || '#9db8cf');
  color = window.normalizeHexColor(color || '#9db8cf', '#9db8cf');
...
window.syncSkullParticleColors = function() {
  ...
  var strength = window.clampRange(Number(tint.strength) || 0, 0, custom ? 0.99 : 0.78);
...
window.createSkullParticleLayer = function() {
  ...
  function noseCut(x, y, z) {
    ...
    var t = window.clampRange((-0.12 - y) / 0.50, 0, 1);
  ...
  for (var j = 0; j < 1450; j++) {
    ...
    var v = Math.random();
    var y = -1.16 + v * 0.48;
    var taper = window.clampRange((y + 1.16) / 0.48, 0, 1);
  ...
  var mat = new THREE.ShaderMaterial({
    uniforms: {
      ...
      uPixel: window.uniforms.uPixel,
      uBass: window.uniforms.uBass,
      uMid: window.uniforms.uMid,
      uTreble: window.uniforms.uTreble,
      uBeat: window.uniforms.uBeat,
      uJawOpen: { value: 0 },
      uSkullFlash: { value: 0 },
      uPointScale: window.uniforms.uPointScale,
      uBloomStrength: window.uniforms.uBloomStrength,
      uColorBoost: window.uniforms.uColorBoost,
      uOpacity: { value: 0 },
...
window.resetSkullPresetView = function(immediate, opts) {
  ...
  if (!opts.keepLyricLock && typeof window.stageLyrics !== 'undefined' && window.stageLyrics && window.stageLyrics.group && window.stageLyrics.group.userData) window.stageLyrics.group.userData.skullMouthLocked = false;
  ...
  if (window.camera && typeof setSkullCameraTargetVectors === 'function') {
    ...
    window.camera.position.copy(skullCameraTargetPos);
...
window.applySkullCameraPose = function(dt) {
  ...
  window.camera.position.lerp(skullCameraTargetPos, skullCameraBlend);
...
window.updateSkullParticleLayer = function(dt) {
  ...
  skullParticleGroup.material.uniforms.uOpacity.value = skullParticleOpacity * window.clampRange(0.78 + (window.fx.intensity || 0.85) * 0.18, 0.56, 1.0);
  var beatTransient = window.clampRange(Math.max(0, window.beatPulse - 0.16) / 0.84, 0, 1.35);
  var flashTarget = window.clampRange(Math.pow(beatTransient, 1.34) * 1.08 + Math.max(0, window.bass - 0.60) * 0.18 * beatTransient, 0, 1);
  ...
  var jawTarget = window.clampRange(0.60 + (0.5 + 0.5 * Math.sin(window.uniforms.uTime.value * 0.50)) * 0.050 + window.bass * 0.060 + skullBeatFlash * 0.090, 0.52, 0.88);
  ...
  var shelfComposition = isSkullShelfCompositionActive();
  var shelfMix = window.clampRange(skullShelfCameraMix || (shelfComposition ? 1 : 0), 0, 1);
  var drift = skullBreathOffset(window.uniforms.uTime.value, shelfComposition);
  var ampTarget = window.clampRange(window.bass * 0.006 + window.mid * 0.004 + skullBeatFlash * 0.070, 0, 0.090);
  skullAmpPulse += (ampTarget - skullAmpPulse) * Math.min(1, dt * (ampTarget > skullAmpPulse ? 11.0 : 4.0));
  var shelfScale = 3.02;
  var targetScale = (SKULL_MODEL_SCALE + (shelfScale - SKULL_MODEL_SCALE) * shelfMix) * (1 + skullAmpPulse) * window.clampRange(1 - skullWheelZoom * 0.055, 0.92, 1.08);
  var shelfX = -1.18;
  var shelfY = 0.32;
...
window.updateRipples = function(dt) {
  ...
  if (isBassHit && (now - lastRippleAt) > RIPPLE_COOLDOWN) {
    ...
    for (var k = 0; k < count; k++) {
      ...
      var str = 0.65 + window.bass * 1.4 + Math.random() * 0.25;
  ...
  window.uniforms.uRippleCount.value = active;
}
...
window.queueAIDepthForCover = function(srcCanvas, edgeCanvas, token, opts, cacheSeed, force) {
  ...
  scheduleVisualApply(async function(){
    if (!window.fx.aiDepth || token !== coverProcessToken || !coverApplyStillCurrent(opts)) return;
...
window.startColorMixTween = function(durationMs) {
  ...
  window.uniforms.uColorMixT.value = 0;
  function step(now) {
    var t = Math.min(1, (now - start) / durationMs);
    t = visualEase(t);
    window.uniforms.uColorMixT.value = t;
...
window.tweenParticleAlpha = function(from, to, durationMs) {
  ...
  function step(now) {
    var t = Math.min(1, (now - start) / durationMs);
    t = t * t * (3 - 2 * t);
    window.uniforms.uAlpha.value = from + (to - from) * t;
...
window.tweenFloatAlpha = function(from, to, durationMs) {
  ...
  function step(now) {
    var t = Math.min(1, (now - start) / durationMs);
    t = t * t * (3 - 2 * t);
    window.uniforms.uFloatAlpha.value = from + (to - from) * t;
...
window.revealIdleParticles = function(target, durationMs) {
  ...
  var from = window.uniforms.uFloatAlpha.value || 0;
...
window.tweenLoading = function(to, durationMs, onComplete) {
  ...
  if (isHiddenForBackgroundOptimization() || isDeepBackgroundMode()) {
    window.uniforms.uLoading.value = to;
    loadingTween = null;
    if (onComplete) onComplete();
    return;
  }
  var start = performance.now();
  var from = window.uniforms.uLoading.value;
  function step(now) {
    var t = Math.min(1, (now - start) / durationMs);
    var eased = visualEase(t);
    window.uniforms.uLoading.value = from + (to - from) * eased;
...
window.hideLoading = function() {
  ...
  loadingHideTimer = setTimeout(function(){
    ...
    if (current <= 0.015 || isHiddenForBackgroundOptimization() || isDeepBackgroundMode()) {
      ...
      window.uniforms.uLoading.value = 0;
      return;
    }
...
window.setCoverDepthState = function(depthTo, aiTo, durationMs) {
  ...
  durationMs = Math.max(1, durationMs || 1);
  var depthFrom = window.uniforms.uHasDepth.value || 0;
  var aiFrom = window.uniforms.uAiBoost.value || 0;
  if (durationMs <= 1 || (Math.abs(depthFrom - depthTo) < 0.001 && Math.abs(aiFrom - aiTo) < 0.001)) {
    window.uniforms.uHasDepth.value = depthTo;
    window.uniforms.uAiBoost.value = aiTo;
    return;
  }
  var start = performance.now();
  function step(now) {
    var t = Math.min(1, (now - start) / durationMs);
    var eased = visualEase(t);
    window.uniforms.uHasDepth.value = depthFrom + (depthTo - depthFrom) * eased;
    window.uniforms.uAiBoost.value = aiFrom + (aiTo - aiFrom) * eased;
...
window.applyCoverCanvas = function(cv, thumbSrc, opts) {
  ...
  if (window.uniforms.uHasCover.value > 0.5 && coverTex.image) {
  ...
  window.uniforms.uHasCover.value = 1;
  if (cachedDepth && cachedDepth.canvas) {
    ...
    setCoverDepthState(opts.deferHeavy ? (window.uniforms.uHasDepth.value > 0.5 ? 0.22 : 0) : 0, opts.deferHeavy ? 0.20 : 0, opts.deferHeavy ? 120 : 1);
  }
  ...
  if (window.shelfManager) window.shelfManager.onCoverChange(thumbSrc);

  // 启动颜色渐变 (1.4 秒)
  var colorMixMs = opts.colorMixDuration || (window.fx.preset === 0 ? 520 : 1400);
  startColorMixTween(opts.fromResolutionChange ? (window.fx.preset === 0 ? 300 : 520) : colorMixMs);
