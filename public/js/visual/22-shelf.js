...
window.shelfLayoutProfile = function() {
  ...
  var detailScale = portrait ? window.clampRange(innerWidth / 820, 0.70, 0.86) : (narrow ? 0.92 : 1.04);
...
window.canShowShelfHoverCueAt = function(e) {
  ...
  if (window.visualGuideActive || emptyHomeActive || homeForcedOpen) return false;
  ...
  if (window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) return false;
...
window.setShelfPinnedOpen = function(open, immediate) {
  ...
  if (hint) hint.classList.toggle('shelf-hidden', window.shelfPinnedOpen || !!(window.shelfManager && window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()));
...
window.clearShelfPreviewOnPointerExit = function() {
  ...
  if (hasContent && window.shelfManager.closeContent) window.safeShelfCloseContent('shelf-mode-reset');
  if (window.shelfPinnedOpen) window.setShelfPinnedOpen(false, true);
  shelfVisibility = 0;
...
window.makeShelfManager = function() {
  ...
  function splitPlaylists() {
    var mine = [], fav = [];
    window.userPlaylists.forEach(function(pl) {
  ...
  function shelfShowsPodcasts() {
    return !window.fx || window.fx.shelfShowPodcasts !== false;
  }

  function shelfMergesCollections() {
    return !!(window.fx && window.fx.shelfMergeCollections === true);
  }
  ...
  function activePlaylists() {
    ...
    if (!window.source.length && shelfPane === 'mine' && panes.fav.length) source = panes.fav;
    if (!window.source.length && shelfPane === 'fav' && panes.mine.length) source = panes.mine;
    return window.source;
  }

  function currentItems() {
    if (window.hasAnyPlatformLogin() && (window.userPlaylists.length || window.myPodcastCollections.length)) {
      var source = activePlaylists();
      var items = window.source.map(function(pl){
        var provider = pl.provider === 'qq' ? 'qq' : 'netease';
        var sourceLabel = provider === 'qq' ? 'QQ' : 'NE';
        return { type:'window.playlist', title: pl.name, sub:sourceLabel + ' · ' + (pl.trackCount||0)+' 首 · 播放 '+window.compactCount(pl.playCount||0),
          cover: pl.cover || '', tag: pl.subscribed ? '收藏歌单' : '我的歌单', playlistId: (provider === 'qq' ? 'qq:' : '') + pl.id, provider: provider };
      });
      if (shelfShowsPodcasts() && (shelfPane === 'mine' || shelfMergesCollections()) && window.myPodcastCollections.length) {
        window.myPodcastCollections.forEach(function(pc){
      ...
      try {
        var localPls = typeof loadLocalPlaylists === 'function' ? window.loadLocalPlaylists() : [];
        if (localPls.length) {
        ...
        var likedSongs = typeof getLocalLikedSongs === 'function' ? window.getLocalLikedSongs() : [];
    ...
    if (window.playQueue.length) {
      return window.playQueue.map(function(song, idx){
        return { type:'queue', title: song.name, sub: song.artist || '未知歌手',
          cover: window.songCoverSrc(song, 360), tag: idx === window.currentIdx ? '正在播放' : ('#' + (idx+1)), queueIndex: idx };
      });
    }
    return [];
  }
  ...
  function cardDrawSignature(card, item) {
    item = item || {};
    var rec = item.cover ? window.playlistCoverCache[item.cover] : null;
    var coverState = item.cover ? (rec && rec.loaded ? 'ready' : (rec && rec.failed ? 'fail' : 'wait')) : 'none';
    var pulseBucket = card && card.isCenter ? Math.round((window.bass + window.beatPulse * 0.85) * 6) : 0;
    return [
      ...
      card && card.dofBucket == null ? -1 : card.dofBucket, pulseBucket, window.shelfAccentHex(), shelfSettings().bgOpacity
    ].join('|');
  }
  ...
  function drawCard(card, item) {
    ...
    if (isNow) {
      ctx.strokeStyle = window.shelfAccentRgba(0.72);
      ctx.lineWidth = 1.8 + Math.sin(window.uniforms.uTime.value * 3) * 0.28 + window.bass * 1.2;
    } else {
    ...
    if (card.selected) {
      ...
      ctx.shadowColor = window.shelfAccentRgba(0.58);
      ctx.shadowBlur = 18;
      ctx.strokeStyle = window.shelfAccentRgba(0.72);
      ctx.lineWidth = 2.2;
      ctx.stroke();
    ...
    if (item.cover) {
      var rec = window.playlistCoverCache[item.cover];
    ...
    ctx.fillStyle = isNow ? window.shelfAccentRgba(0.92) : 'rgba(255,255,255,0.92)';
    ...
    // 律动进度条
    ctx.strokeStyle = isNow ? window.shelfAccentRgba(0.90) : 'rgba(255,255,255,0.30)';
    ctx.lineWidth = 3.5;
    ctx.beginPath();
    ctx.moveTo(tx, H - pad - 22);
    ctx.lineTo(tx + Math.min(260, 80 + window.bass * 320), H - pad - 22);
    ctx.stroke();

    if (card.isCenter) {
      var actionY = H - pad - 78;
      if (item.type === 'window.playlist' || item.type === 'localPlaylist' || item.type === 'localLiked') {
        ...
        playGrad.addColorStop(0.55, window.shelfAccentRgba(0.94));
        playGrad.addColorStop(1, window.shelfAccentRgba(0.58));
        ctx.fillStyle = playGrad; ctx.fill();
        ctx.strokeStyle = window.shelfAccentRgba(0.44);
        ctx.lineWidth = 1.1; ctx.stroke();
        ctx.font = '800 14px Inter, "Microsoft YaHei", Arial';
        ctx.fillStyle = readableInkForHex(window.shelfAccentHex());
        ctx.fillText(btnLabel, tx + 25, actionY + 24);

        if (item.type === 'window.playlist') {
        ...
        ctx.fillStyle = window.shelfAccentRgba(0.84);
  ...
  function buildOneCard(item, i) {
    ...
    mesh.userData.action = item.type === 'window.playlist'
      ...
      : (item.type === 'podcastCollection'
        ...
        : (item.type === 'localPlaylist'
          ...
          : (item.type === 'localLiked'
            ...
            : (item.type === 'queue' ? { kind:'window.playQueue', index: item.queueIndex } : { kind:'empty' }))));
    group.add(mesh);
  ...
  function warmTextureUpload(tex) {
    if (!tex || !window.renderer || typeof window.renderer.initTexture !== 'function') return;
    try { window.renderer.initTexture(tex); } catch (e) {}
  }
  ...
  function rebuild(asyncCards) {
    ...
    if (allItems.length && allItems[0].type === 'queue' && window.currentIdx >= 0) {
      centerTarget = Math.min(allItems.length - 1, window.currentIdx);
  ...
  function placeCard(card, i, totalCards, modeIs) {
    ...
    var parX = window.pointerParallax.x || 0;
    var parY = window.pointerParallax.y || 0;
    ...
    if (modeIs === 'side') {
      ...
      var nowT = window.uniforms.uTime.value;
      var hoverBreath = (!window.shelfPinnedOpen && !detailOpenSide) ? shelfVisibility : 0;
      var passiveAlways = window.shelfAlwaysVisible() && !window.shelfPinnedOpen && !detailOpenSide;
      ...
      if (skullShelfPose && window.camera) {
        card.mesh.quaternion.copy(window.camera.quaternion);
      ...
      var paneRawS = Math.max(0, Math.min(1, (window.uniforms.uTime.value - paneSwitchAt - absD * 0.030) / 0.72));
  ...
  function playPlaylistCard(card) {
    ...
    openCardIdx = -1;
    window.setShelfPinnedOpen(false, true);
    if (typeof setFocusZone === 'function') setFocusZone(null, true);
    window.loadPlaylistIntoQueueById(action.playlistId, true, action.title || (card.item && card.item.title) || '');
    return true;
  }
  ...
  function createStageExtras() {
    ...
    var pmat = new THREE.ShaderMaterial({
      uniforms:{ uTime: window.uniforms.uTime, uPixel: window.uniforms.uPixel, uDotTex: window.uniforms.uDotTex },
...
void main(){ vec4 t = texture2D(uDotTex, gl_PointCoord); if (t.a < 0.02) discard; gl_FragColor = vec4(vC, t.a * vA); }`,
    ...
    if (group.parent) group.parent.add(connectorParticles); else window.scene.add(connectorParticles);
    // 底部地面反射
    ...
    if (group.parent) group.parent.add(floorMirror); else window.scene.add(floorMirror);
  }
  ...
  function sig(items) {
    if (window.hasAnyPlatformLogin() && (window.userPlaylists.length || window.myPodcastCollections.length)) {
      ...
      return [
        ...
        window.source.length,
        window.myPodcastCollections.length,
        sampleItems.map(function(it){
          return [it.type || '', it.playlistId || '', it.podcastKey || '', it.title || '', it.sub || '', it.tag || ''].join('|');
        }).join('||')
      ].join('::');
    }
    items = items || window.playQueue.map(function(song, idx){
      return { type:'queue', title: song.name, queueIndex: idx };
    });
    var sample = items.slice(0, 3).concat(items.slice(Math.max(3, items.length - 3)));
    return ['queue', items.length, window.currentIdx, sample.map(function(it){ return [it.type, it.playlistId||'', it.queueIndex||'', it.title||''].join('|'); }).join('||')].join('::');
  }

  function switchPane(nextPane) {
    if (shelfMergesCollections()) return false;
    if (nextPane === window.shelfPane) return false;
    paneMemory[window.shelfPane] = Math.max(0, Math.round(centerTarget));
    shelfPane = nextPane;
    collectionReveal = 0;
  ...
  function step(direction) {
    ...
    if (!shelfMergesCollections()) {
      if (window.hasAnyPlatformLogin() && window.userPlaylists.length && shelfPane === 'mine' && atEnd && panes.fav.length) {
        collectionReveal += Math.min(1.5, Math.abs(direction));
        if (collectionReveal >= 3) switchPane('fav');
        return;
      }
      if (window.hasAnyPlatformLogin() && window.userPlaylists.length && shelfPane === 'fav' && atStart && panes.mine.length) {
        collectionReveal += Math.min(1.5, Math.abs(direction));
  ...
  function screenHitCard(card, sx, sy, pad) {
    ...
    for (var i = 0; i < pts.length; i++) {
      pts[i].applyMatrix4(card.mesh.matrixWorld).project(window.camera);
    ...
    var u = window.clampRange((sx - minX) / Math.max(1, maxX - minX), 0, 1);
    var v = 1 - window.clampRange((sy - minY) / Math.max(1, maxY - minY), 0, 1);
    return { x: u, y: v };
  }
  ...
  return {
    setMode: function(m) {
      ...
      mode = m;
      if (m === 'off') {
        if (group) { window.scene.remove(group); cards.forEach(function(c){ c.texture.dispose(); c.mesh.material.dispose(); c.mesh.geometry.dispose(); }); }
        if (connectorParticles) { window.scene.remove(connectorParticles); connectorParticles.geometry.dispose(); connectorParticles.material.dispose(); connectorParticles = null; }
        if (floorMirror) { window.scene.remove(floorMirror); floorMirror.geometry.dispose(); floorMirror.material.dispose(); floorMirror = null; }
        group = null; cards = [];
        if (contentList) contentList.close();
        return;
      }
    ...
    update: function(dt) {
      ...
      var px = window.pointerParallax.x, py = window.pointerParallax.y;
      ...
      if (!appRevealed) {
        ...
        else targetVis = (contentOpen || window.shelfPinnedOpen || window.shelfAlwaysVisible()) ? 1.0 : (cueVis > 0.01 ? Math.max(0.16, cueVis * 0.88) : 0);
      } else {
      ...
      if (mode === 'side') {
        var passiveAlwaysGroup = window.shelfAlwaysVisible() && !window.shelfPinnedOpen && !(contentList && contentList.isOpen());
        ...
        var bindToCover = window.shelfAlwaysVisible() && particles && particles.rotation && !(contentList && contentList.isOpen());
        if (bindToCover) {
      ...
      // 内容更新 (节流)
      if (window.uniforms.uTime.value - lastUpdate > 0.8) {
        lastUpdate = window.uniforms.uTime.value;
        var nextSig = sig();
        if (nextSig !== lastSig) rebuild();
        else {
          var pulseBucket = Math.round((window.bass + window.beatPulse * 0.85) * 10);
          var redrawInterval = window.playing ? 1.35 : 4.0;
          if (pulseBucket !== lastCardPulseBucket || window.uniforms.uTime.value - lastCardRedrawAt > redrawInterval) {
            lastCardPulseBucket = pulseBucket;
            lastCardRedrawAt = window.uniforms.uTime.value;
            cards.forEach(function(c){
              c.item = allItems[c.index] || c.item;
              c.isCenter = Math.abs(c.index - centerSmooth) < 0.5;
              if (c.isCenter || c.dofBucket <= 1 || c.index === window.currentIdx) drawCard(c, c.item);
            });
          }
        }
      }
    ...
    onCoverChange: function() {
      if (group && mode !== 'off' && window.uniforms.uTime.value - lastUpdate > 0.2) {
        lastUpdate = window.uniforms.uTime.value;
        rebuild();
      }
    },
    ...
    triggerAction: function(action) {
      ...
      if (action.kind === 'window.playQueue') {
        window.playQueueAt(action.index);
        ...
        if (typeof showLocalPlaylistDetail === 'function') window.showLocalPlaylistDetail(action.playlistId);
      } else if (action.kind === 'loadLocalLiked') {
        if (typeof showLocalPlaylistDetail === 'function') window.showLocalPlaylistDetail('__liked__');
    ...
    openContent: function(cardIdx) {
      ...
      if (action.kind === 'empty') window.togglePlaylistPanel(true);
    },
    closeContent: function() {
      openCardIdx = -1;
      if (contentList) contentList.close();
      var hint = document.getElementById('hint');
      if (hint) hint.classList.toggle('shelf-hidden', window.shelfPinnedOpen);
      if (typeof setFocusZone === 'function') setFocusZone(window.shelfPinnedOpen ? 'shelf-side' : null, true);
      if (typeof updateEmptyHomeVisibility === 'function') updateEmptyHomeVisibility({ forceLoad: false });
    },
...
window.scheduleShelfRebuild = function(reason, asyncCards) {
  ...
  deferredShelfRebuild.raf = requestAnimationFrame(function(){
    ...
    scheduleUiWarmTask(function(){
      ...
      window.safeShelfRebuild(deferredShelfRebuild.reason, deferredShelfRebuild.asyncCards);
    }, 260);
  });
}
...
window.makeContentListManager = function() {
  ...
  var playlistTitle = '';
  var contentKind = 'window.playlist';
  var sourceCard = null;
  var requestToken = 0;
  var openAnimAt = -10;
  var rowAnimAt = -10;
  ...
  function canvasAccent(alpha, fallback) {
    return window.shelfAccentRgba(alpha, fallback);
  }
  ...
  function drawPanel() {
    ...
    ctx.fill();
    if (coverUrl) {
      var coverRec = window.playlistCoverCache[coverUrl];
    ...
    var sweep = (Math.sin((window.uniforms.uTime.value || 0) * 1.7) + 1) * 0.5;
  ...
  function drawPanelIfNeeded(force, nowT) {
    nowT = nowT == null ? (window.uniforms.uTime.value || 0) : nowT;
  ...
  function drawRow(row, song, isCenter) {
    ...
    var songCover = window.songCoverSrc(song, 80);
    ...
    if (actionReady || hasSongCover) {
      ...
      ctx.fill();
      if (hasSongCover) {
        var songCoverRec = window.playlistCoverCache[songCover];
    ...
    if (loadingRow) {
      ...
      var phase = ((window.uniforms.uTime.value || 0) * 0.85) % 1;
  ...
  function startRowsLoadedIntro() {
    rowAnimAt = window.uniforms.uTime.value;
    panelDirty = true;
    rowsDirty = true;
  ...
  function syncRenderedRows(force) {
    if (!group) return;
    var nowT = window.uniforms.uTime.value || 0;
  ...
  return {
    ...
    open: async function(playlistId, title, fromCard) {
      ...
      openAnimAt = window.uniforms.uTime.value;
      ...
      var openDynamicDetail = !openSkullDetail && shouldUseShelfDynamicCamera('shelf-detail') && window.camera;
      ...
      if ((openSkullDetail || openDynamicDetail) && window.camera) {
        group.quaternion.copy(window.camera.quaternion);
      ...
      contentKind = podcastCollectionKey ? 'podcast' : 'window.playlist';
      // 拉取歌单/播客集合
      var r = null;
      try {
        r = podcastCollectionKey
          ? await window.neteasePodcastMyItems(podcastCollectionKey, 36)
          : (qqPlaylistId
            ? await window.qqPlaylistTracks(qqPlaylistId)
            : await window.neteasePlaylistTracks(playlistId));
      } catch (e) {
      ...
      try {
        ...
        window.showToast('歌单已载入，3D列表刷新失败');
      }
    },
    close: function() {
      open = false;
    ...
    update: function(dt) {
      ...
      var layout = detailLayout();
      var skullDetail = shouldUseSkullSafeShelfCamera();
      var dynamicDetail = !skullDetail && shouldUseShelfDynamicCamera('shelf-detail') && window.camera;
      ...
      if (panel && panel.mesh) {
        var pr = Math.max(0, Math.min(1, (window.uniforms.uTime.value - openAnimAt) / 0.72));
        pr = pr * pr * (3 - 2 * pr);
    ...
    pickRowAtScreen: function(sx, sy) {
      ...
      for (var ri = 0; ri < ordered.length; ri++) {
        ...
        for (var pi = 0; pi < pts.length; pi++) {
          pts[pi].applyMatrix4(row.mesh.matrixWorld).project(window.camera);
    ...
    screenContainsPanel: function(sx, sy) {
      ...
      for (var pi = 0; pi < pts.length; pi++) {
        pts[pi].applyMatrix4(panel.mesh.matrixWorld).project(window.camera);
        var x = (pts[pi].x + 1) * innerWidth / 2;
        var y = (1 - pts[pi].y) * innerHeight / 2;
        minX = Math.min(minX, x); maxX = Math.max(maxX, x);
        minY = Math.min(minY, y); maxY = Math.max(maxY, y);
    ...
    rowActionAtScreen: function(row, sx, sy) {
      ...
      for (var i = 0; i < corners.length; i++) {
        corners[i].applyMatrix4(row.mesh.matrixWorld).project(window.camera);
      ...
      var u = window.clampRange((sx - minX) / w, 0, 1);
      var v = window.clampRange((sy - minY) / h, 0, 1);
    ...
    playRow: function(row) {
      ...
      if (row.song && row.song.type === 'podcast-radio') {
        ...
        var smRadio = window.shelfManager;
        if (smRadio) window.safeShelfCloseContent('content-play-podcast-radio');
        return;
      }
      ...
      var allSongs = allTracks.filter(function(song){ return song && song.id; }).map(function(song){
        return window.cloneSong(song);
      });
      if (!allSongs.length || playIndex < 0) return;
      playQueue = allSongs;
      currentIdx = playIndex;
      window.safeRenderQueuePanel('content-play-row');
      window.safeShelfRebuild('content-play-row');
      window.forcePlaybackControlsInteractive();
      window.playQueueAt(playIndex, { preserveHomeState: true }).catch(function(e){
        console.warn('[ContentPlayRow]', e);
      });
      // 关闭内容框
      var sm = window.shelfManager;
      if (sm) window.safeShelfCloseContent('content-play-row');
    }
  };
...
window.requestPlaylistCover = function(url, cb) {
  ...
  rec = window.playlistCoverCache[url] = { loaded:false, loading:true, waiters: cb ? [cb] : [], img:null, failed:false };
  var img = new Image();
  if (!window.isInlineCoverSrc(url)) img.crossOrigin = 'anonymous';
  img.onload = function(){
  ...
  var src = window.coverProxySrc(url);
  if (!src) {
    rec.loading = false; rec.failed = true;
    rec.waiters.splice(0).forEach(function(fn){ setTimeout(function(){ fn(null); }, 0); });
    return;
...
window.updateShelfCardHoverSelection = function(e) {
  ...
  var mode = window.shelfManager.getMode && window.shelfManager.getMode();
  if (!mode || mode === 'off') {
    window.shelfManager.clearSelected();
    return;
  }
  if (window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) {
    window.shelfManager.clearSelected();
    return;
  }
  var canInteract = window.shelfManager.canInteract && window.shelfManager.canInteract();
  if (!canInteract) {
    window.shelfManager.clearSelected();
    return;
  }
  if (mode === 'side') {
    if (!window.shelfPinnedOpen && window.shelfAlwaysVisible()) {
      var alwaysHit = pointerCardHit(raycasterFromPointerEvent(e), e, 18);
      if (alwaysHit && alwaysHit.card) window.shelfManager.setSelected(alwaysHit.card.index);
      else window.shelfManager.clearSelected();
      return;
    }
    var sideUsable = window.shelfPinnedOpen || shelfAutoHiddenInputReady();
    if (!sideUsable) {
      window.shelfManager.clearSelected();
      return;
    }
...
window.renderer.domElement.addEventListener('click', function(e){
  if (!window.shelfManager || window.shelfManager.getMode() === 'off') return;
  if (document.body.classList.contains('splash-active')) return;
  if (isPointerOverUi(e)) return;
  if (mouseDownAt.hadDrag) { mouseDownAt.hadDrag = false; return; }

  var rc = raycasterFromPointerEvent(e);
  var mode = window.shelfManager.getMode();
  var canInteract = window.shelfManager.canInteract && window.shelfManager.canInteract();

  // 优先二级内容框
  if (window.shelfManager.hasOpenContent()) {
    var cl = window.shelfManager.getContentList && window.shelfManager.getContentList();
    if (cl) {
      ...
      if (rowHit) {
        ...
        if (selectedRow && !rowIsPodcastRadio && hitLikeButton) {
          window.toggleLikeDetailSong(rowHit.row.song);
        } else if (selectedRow && !rowIsPodcastRadio && hitCollectButton) {
          window.collectDetailSong(rowHit.row.song);
      ...
      var returnHit = window.shelfManager.raycastCards(rc);
      window.safeShelfCloseContent('shelf-card-return');
      if (mode === 'side') window.setShelfPinnedOpen(true, true);
      if (returnHit && returnHit.card) {
        window.shelfManager.scrollBy(returnHit.card.index - window.shelfManager.getCenterIdx());
      }
      return;
    }
  }
  ...
  // 一级卡片
  var hit = pointerCardHit(rc, e, mode === 'side' && !window.shelfPinnedOpen && window.shelfAlwaysVisible() ? 18 : undefined);
  if (mode === 'side' && !window.shelfPinnedOpen && !canUseSideShelfWithoutPinnedOpen()) return;

  if (hit) {
    if (mode === 'side') window.setShelfPinnedOpen(true, true);
    var idx = hit.card.index;
    if (Math.abs(idx - window.shelfManager.getCenterIdx()) < 0.5) {
      if (isShelfPlaylistPlayHit(hit) && window.shelfManager.playPlaylistAt && window.shelfManager.playPlaylistAt(idx)) return;
      window.shelfManager.openContent(idx);
    } else {
      window.shelfManager.scrollBy(idx - window.shelfManager.getCenterIdx());
    }
  } else if (mode === 'side' && window.shelfPinnedOpen) {
    window.setShelfPinnedOpen(false, true);
  }
});

window.renderer.domElement.addEventListener('contextmenu', function(e){
  if (document.body.classList.contains('splash-active')) return;
  if (isPointerOverUi(e)) return;
  e.preventDefault();
  e.stopPropagation();
  if (typeof suppressBottomControlsForShelf === 'function') suppressBottomControlsForShelf(980);
  if (!window.shelfManager) return;
  var mode = window.shelfManager.getMode && window.shelfManager.getMode();
  if (mode === 'off') {
    setShelfMode('side');
    mode = 'side';
  ...
  if (window.shelfManager.hasOpenContent && window.shelfManager.hasOpenContent()) {
    ...
    window.safeShelfCloseContent('shelf-context-toggle');
    window.setShelfPinnedOpen(true, true);
    return;
  }
  window.setShelfPinnedOpen(!window.shelfPinnedOpen, true);
  if (!window.shelfPinnedOpen && typeof setFocusZone === 'function') setFocusZone(null, true);
});
...
window.renderer.domElement.addEventListener('wheel', function(e){
  if (isPointerOverUi(e)) return;
  if (!window.shelfManager || window.shelfManager.getMode() === 'off') return;
  window.markRenderInteraction('shelf-wheel', 900);
  var rc = raycasterFromPointerEvent(e);
  // 二级框打开时, 只有真正命中详情行才接管滚轮
  if (window.shelfManager.hasOpenContent()) {
    var cl = window.shelfManager.getContentList();
    if (cl) {
      var rowHit = cl.raycastRows(rc);
  ...
  var canScrollShelf = window.shelfManager.canInteract && window.shelfManager.canInteract();
  var shelfPreviewActive = shelfAutoHiddenInputReady();
  var cardWheelHit = canScrollShelf ? pointerCardHit(rc, e, mode === 'side' && !window.shelfPinnedOpen && window.shelfAlwaysVisible() ? 18 : undefined) : null;
  if (canScrollShelf && e.shiftKey && (mode !== 'side' || window.shelfPinnedOpen || shelfPreviewActive || window.shelfAlwaysVisible())) inShelfArea = true;
  else if (canScrollShelf && mode === 'side') {
    if (window.shelfPinnedOpen) inShelfArea = isShelfWheelZone(e) || !!cardWheelHit;
    else if (window.shelfAlwaysVisible()) inShelfArea = !!cardWheelHit;
  ...
  if (inShelfArea) {
    ...
    window.shelfManager.scrollBy(e.deltaY > 0 ? 1 : -1);
  }
...
window.consumeFreeCameraKeyEvent = function(e, isDown) {
  ...
  if (!window.freeCamera || !window.freeCamera.active) return false;
  ...
  e.preventDefault();
  e.stopImmediatePropagation();
  window.freeCamera.keys = window.freeCamera.keys || {};
  window.freeCamera.keys[e.code] = !!isDown;
  window.markRenderInteraction('free-window.camera-key', 900);
  return true;
}
...
document.addEventListener('keydown', function(e){
  ...
  window.markRenderInteraction('keyboard', 700);
  if (e.code === 'KeyK') {
    e.preventDefault();
    if (window.freeCamera && (window.freeCamera.active || window.freeCamera.locked)) resetFreeCameraToDefault();
    else {
      recenterCamera();
      window.showToast('镜头已回正');
    }
    return;
  }
  if (e.code === 'KeyR') {
  ...
  if (window.freeCamera && window.freeCamera.active) {
    if (/^(KeyW|KeyA|KeyS|KeyD|KeyQ|KeyE|Space|ShiftLeft|ShiftRight|ControlLeft|ControlRight)$/.test(e.code)) {
      e.preventDefault();
      e.stopImmediatePropagation();
      window.freeCamera.keys[e.code] = true;
      return;
    }
  }
  // 歌词偏移 Alt+[ Alt+]
  if (e.altKey && (e.code === 'BracketLeft' || e.code === 'BracketRight')) {
    if (e.repeat) return;
    e.preventDefault();
    if (_lyricOffset === undefined) _lyricOffset = 0;
    _lyricOffset = Math.max(-30, Math.min(30, window._lyricOffset + (e.code === 'BracketLeft' ? -0.5 : 0.5)));
    window.showLyricOffsetToast();
    return;
  }
  if (!window.shelfManager) return;
  if (e.code === 'BracketRight' || e.code === 'PageDown') window.shelfManager.next();
  else if (e.code === 'BracketLeft' || e.code === 'PageUp') window.shelfManager.prev();
});
document.addEventListener('keyup', function(e){
  if (!window.freeCamera || !window.freeCamera.keys) return;
  if (/^(KeyW|KeyA|KeyS|KeyD|KeyQ|KeyE|Space|ShiftLeft|ShiftRight|ControlLeft|ControlRight)$/.test(e.code)) {
    window.freeCamera.keys[e.code] = false;
  }
});
window.addEventListener('blur', function(){
  if (window.freeCamera && window.freeCamera.keys) window.freeCamera.keys = {};
});
